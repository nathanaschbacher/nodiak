...
HTTPBackend.prototype.request = function(method, query, _return) {
  var req = http.request({ headers: HTTPBackend.appendDefaultHeaders(HTTPBackend.metadataToHeaders(query.headers)),
...
HTTPBackend.metadataToHeaders = function(metadata) {
  var headers = munchMetadata(metadata, "", temp_obj);
...
function munchMetadata(metadata, accumulator, container) {
