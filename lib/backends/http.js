...
HTTPBackend.headersToMetadata = function(headers) {
  var metadata = munchHeaders(headers);
  
  if(metadata.index) {
    metadata.index = munchIndexes(metadata.index);
  }
...
function munchHeaders(headers) {
  ...
  for(var key in headers) {
    ...
    if(low_key.indexOf("x-riak-")) {
      ...
      low_key = low_key.slice(7);
    }
    
    var parts = low_key.split("-");
    parts.push(headers[key]);
    metadata = toNamespaced(metadata, parts);
  }
  return metadata;
}

function munchIndexes(indexes) {  
  var temp = {};
  for(var index in indexes) {
    var _at = index.lastIndexOf("_");

    var name = index.substring(0, _at);
    var type = index.substring(_at+1);
    var value = indexes[index].split(', ');

    temp[name] = { type: type, value: value};
  }
  
  return temp;
}

function toNamespaced(parent, parts) {
  var namespace = parent;
  
  for(var i = 0, length = parts.length-1; i < length; i++) {
      if(i+1 == length) {
        parent[parts[i]] = parts[i+1];
      }
      else {
