...
HTTPBackend.prototype.request = function(method, query, _return) {
  query.headers = HTTPBackend.appendDefaultHeaders(HTTPBackend.metadataToHeaders(query.headers));
  ...
  query.body = HTTPBackend.defaults.mime[query.headers['content-type']] ? HTTPBackend.defaults.mime[query.headers['content-type']].encoder(query.body) : query.body;
...
HTTPBackend.responseHandler = function(_return, res) { // reversed signature due to bind()'s prepend behavior in request()
  ...
  res.on('end', function() {
    ...
    body = (body_length !== 0 && HTTPBackend.defaults.mime[res.headers['content-type']]) ? HTTPBackend.defaults.mime[res.headers['content-type']].decoder(body) : body;
...
HTTPBackend.autoParse = function(data, mime_type) {
